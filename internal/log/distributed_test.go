...
func TestMultipleNodes(t *testing.T) {
	...
	for i := 0; i < nodeCount; i++ {
		...
		if i != 0 {
			...
			// leader, wait til leader is selected or timeout
	...
	for _, record := range records {
		// append record to leader server
		off, err := logs[0].Append(record)
		require.NoError(t, err)
		require.Eventually(t, func() bool {
			for j := 0; j < nodeCount; j++ {
				// all servers read from offset
				got, err := logs[j].Read(off)
				if err != nil {
					return false
				}
				// offset & record both check off
				record.Offset = off
			...
			return true
			// enough time for replicating before verify
	...
	// check node 1 is left and log out of range
	record, err := logs[1].Read(off)
	require.IsType(t, api.ErrOffsetOutOfRange{}, err)
	require.Nil(t, record)

	// successfully replicated for node 2
